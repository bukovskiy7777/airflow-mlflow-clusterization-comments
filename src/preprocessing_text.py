...
def preprocessing(string, stopwords, stem):
    ...
    string = ' '.join([
        ...
        if len(s.strip()) >= 3 and s.strip() not in stopwords #and s.isalpha()
    ])
    """
    string = ' '.join([
    ...
    # удаляем местоимения и общие наречия и глаголы
    exclude = ['весь', 'который', 'очень', 'свой', 'почему', 'лишь', 'либо', 'это', 'ваш', 
               'хотеть', 'мочь', 'сказать', 'знать', 'просто', 'вообще', 'абсолютно', 'видеть', 'смотреть', 'говорить']
    string = ' '.join([s for s in string.split() if s not in exclude])
