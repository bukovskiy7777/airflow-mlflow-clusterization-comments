...
# Call this before using stopwords
...
config_path = os.path.join(r'/home/oleksandr/apps/airflow-local/config/params_all.yaml')
...
def save_topics(data, y_predict, vector_model, num_words, name_file):
    ...
    # Разворачиваем порядок строк (iloc[::-1]) чтобы наиболее значимые слова были сверху
    df = pd.DataFrame().from_dict(topics)
    df.iloc[::-1].reset_index(drop=True).to_csv(name_file, encoding='utf-8-sig')
    #pd.DataFrame().from_dict(topics).to_csv(name_file, encoding='utf-8-sig')
